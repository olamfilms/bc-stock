...
export default function VideoGrid({ initialCategory, searchQuery = '' }: VideoGridProps) {
  ...
  const fetchItems = useCallback(async (cat: string, off: number) => {
    ...
    // When a search is active we fetch all categories so we can filter client-side
    if (cat && cat !== 'all') {
  ...
  // Reset and reload when category changes (search doesn't trigger a re-fetch)
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    setItems([])
  ...
  return (
    <>
      ...
      {/* Only show Load More when not searching (search already covers all fetched items) */}
